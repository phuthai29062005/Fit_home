...
/* demo chưa có kho sản phẩm/SKU thật để trỏ thẳng vào 1 trang cụ thể (không
   tồn tại) — dẫn thẳng vào kết quả TÌM KIẾM TRÊN SHOPEE theo đúng tên +
   hãng sản phẩm (theo yêu cầu người dùng), thay vì tìm Google chung chung.
   Link tìm kiếm Shopee luôn ra 1 trang thật, không bao giờ 404/sai domain.
   LƯU Ý (giữ lại từ lần cân nhắc trước): với đồ điện máy — tủ lạnh, máy
   giặt, điều hoà, bình nóng lạnh... — kết quả tìm trên Shopee đôi khi là
   hàng trôi nổi/không rõ nguồn gốc, giá lệch khá xa hãng chính hãng (Điện
   Máy Xanh, Nguyễn Kim...). Nếu sau này thấy phiền, có thể tách riêng nhóm
   ELECTRONICS_CATS này sang 1 nguồn khác đáng tin hơn mà không phải sửa lại
   toàn bộ buyUrl. */
const ELECTRONICS_CATS = ['maygiat','tulanh','dieuhoa','binhnonglanh','bepdien','mayhutmui'];
const buyUrl = p => 'https://shopee.vn/search?keyword=' + encodeURIComponent(p.name + ' ' + p.brand.split('·')[0].trim());
...
const OPP_WALL = {N:'S', S:'N', W:'E', E:'W'};
/* ---------- phong thuỷ CẤU TRÚC (cửa chính) — dùng chung cho MỌI mặt bằng ----------
   Suy luận thuần hình học từ plan.entry/room.openings/room.fixtures, không
   hardcode theo id phòng hay id mặt bằng cụ thể nào, để tự áp dụng đúng cho
   bất kỳ căn hộ nào thêm sau này mà không cần sửa lại phần này. */
/* toạ độ GLOBAL (theo hệ trục cả căn hộ) của một opening, thay vì local như
   openingRect() — cần để so khớp 2 opening ở 2 phòng khác nhau. */
function openingGlobalRect(o, r){
  const g = openingRect(o, r);
  return {x:r.x+g.x, y:r.y+g.y, w:g.w, d:g.d};
}
/* phòng chứa cửa chính của cả căn hộ — suy ra bằng cách tìm 1 opening type
   'door' có toạ độ global trùng khớp plan.entry (không giả định id phòng). */
function findEntryRoom(){
  const P = plan(), e = P.entry;
  if (!e) return null;
  for (const r of P.rooms){
    for (const o of (r.openings||[])){
      if (o.type !== 'door') continue;
      const g = openingGlobalRect(o, r);
      const same = (e.wall==='N'||e.wall==='S')
        ? (o.wall===e.wall && Math.abs(g.x-e.pos)<1 && Math.abs(g.w-e.len)<1)
        : (o.wall===e.wall && Math.abs(g.y-e.pos)<1 && Math.abs(g.d-e.len)<1);
      if (same) return {room:r, opening:o};
    }
  }
  return null;
}
function wallLine(room, wall){
  if (wall==='N') return room.y;
  if (wall==='S') return room.y+room.d;
  if (wall==='W') return room.x;
  return room.x+room.w;
}
/* phòng ở "bên kia" một opening cho trước — suy ra bằng cách tìm 1 phòng khác
   có tường ĐỐI DIỆN đúng trùng vị trí (chung tường vật lý) VÀ có 1 opening ở
   đúng tường đó với đoạn toạ độ chồng lên opening đang xét — không cần khai
   báo quan hệ 2 phòng trong data, nên áp dụng được cho bất kỳ mặt bằng nào. */
function neighborRoomThrough(hostRoom, opening){
  const P = plan(), hg = openingGlobalRect(opening, hostRoom);
  const wantWall = OPP_WALL[opening.wall];
  const hostLine = wallLine(hostRoom, opening.wall);
  for (const r of P.rooms){
    if (r === hostRoom) continue;
    if (Math.abs(wallLine(r, wantWall) - hostLine) > 15) continue; // không chung tường
    for (const o of (r.openings||[])){
      if (o.wall !== wantWall) continue;
      const og = openingGlobalRect(o, r);
      const overlapAxis = (opening.wall==='N'||opening.wall==='S')
        ? (og.x < hg.x+hg.w && hg.x < og.x+og.w)
        : (og.y < hg.y+hg.d && hg.y < og.y+og.d);
      if (overlapAxis) return r;
    }
  }
  return null;
}
/* dải "tầm nhìn thẳng" từ 1 cửa xuyên suốt chiều còn lại của phòng chứa nó —
   dùng để dò "đứng ở cửa nhìn thẳng vào sẽ thấy gì" (bếp / cửa đối diện...). */
function sightlineCorridor(hostRoom, opening){
  const g = openingRect(opening, hostRoom);
  if (opening.wall==='N' || opening.wall==='S') return {x:g.x, y:0, w:g.w, d:hostRoom.d};
  return {x:0, y:g.y, w:hostRoom.w, d:g.d};
}
...
/* gương nào đang áp đúng tường đối diện đầu giường VÀ chắn ngay trước chân
   giường — tách riêng để cả mục kiểm tra lẫn nút "sửa nhanh" dùng chung 1
   nguồn, luôn khớp nhau tuyệt đối (đổi tiêu chí ở đây là đổi cho cả 2 nơi). */
function mirrorsFacingBedFoot(bed, r, footWall){
  const B = rectOf(bed);
  ...
  return itemsIn(r.id).filter(a => byId(a.id).cat === 'guong').filter(m => {
    const M = rectOf(m);
...
/* dời 1 món (theo uid) sang 1 trong các tường cho trước, dùng lại đúng máy
   tìm chỗ trống (placeOnWall) của thuật toán tự động — không tìm được chỗ
   thì giữ nguyên vị trí cũ, không bao giờ làm mất đồ. */
function moveItemToWalls(uid, walls, opt){
  const arr = items(), idx = arr.findIndex(i => i.uid === uid);
  if (idx === -1) return false;
  const saved = arr[idx], p = byId(saved.id);
  arr.splice(idx, 1);
  const placed = placeOnWall(p, walls, opt || {gap:20});
  if (!placed){ arr.splice(idx, 0, saved); return false; }
  return true;
}
/* nút "sửa nhanh" cho mục "Đầu giường quay hướng ... — chưa hợp mệnh": tìm
   (các) tường trong phòng có hướng thật hợp cung mệnh (theo "Hướng ban
   công" đã chọn), rồi dời giường sang đó thay vì bắt người dùng tự kéo. */
function fixBedDirection(){
  const bed = items().find(a => byId(a.id).cat === 'giuong');
  if (!bed) return;
  const cm = cungMenh(yearForRoom(S.roomId));
  const goodWalls = ['N','E','S','W'].filter(w => cm.tot.includes(huongCua(w)));
  if (!goodWalls.length) return;
  /* không tìm được chỗ (tường đó bị cửa/cửa sổ chiếm hết) thì đơn giản
     không đổi gì — KHÔNG dùng alert()/confirm(): hộp thoại chặn (blocking)
     làm đứng hình toàn bộ canvas/3D đang chạy nền, trải nghiệm rất tệ. Cảnh
     báo "chưa hợp mệnh" + nút bấm vẫn còn nguyên sau khi renderAll(), tự
     nó đã là phản hồi đủ rõ (bấm mà giường không đổi = không có chỗ). */
  moveItemToWalls(bed.uid, goodWalls, {gap:30, avoidWindow:true, notInFrontOfDoor:true})
    || moveItemToWalls(bed.uid, goodWalls, {gap:30, avoidWindow:true})
    || moveItemToWalls(bed.uid, goodWalls, {gap:12});
  S.sel = null; renderAll();
}
/* nút "sửa nhanh" cho mục "Gương đối diện chân giường": dời các gương đang
   chắn sang tường bên (không phải tường đầu/chân giường). */
function fixMirrorFacingBed(){
  const bed = items().find(a => byId(a.id).cat === 'giuong');
  if (!bed) return;
  const r = room(), e = headEdge(bed);
  const OPP = {N:'S', S:'N', W:'E', E:'W'}, footWall = OPP[e];
  const offenders = mirrorsFacingBedFoot(bed, r, footWall);
  const sideWalls = ['N','S','W','E'].filter(w => w !== footWall && w !== e);
  // không dùng alert() khi không tìm được chỗ — xem lý do ở fixBedDirection()
  // ngay trên; cảnh báo vẫn còn nguyên sau renderAll() là đủ tự giải thích.
  offenders.forEach(m => moveItemToWalls(m.uid, sideWalls, {gap:20, avoidWindow:false}));
  S.sel = null; renderAll();
}

/* 1 thẻ gợi ý sản phẩm nhỏ (tên · giá · nút +Thêm · link Mua) — tách dùng
   chung cho mọi mục kiểm tra có gợi ý mua đồ cụ thể, thay vì chỉ nói suông
   "có sẵn trong danh mục". targetRoomId mặc định là phòng đang xem. */
function suggestPickHtml(p, targetRoomId){
  if (!p) return '';
  const rid = targetRoomId || S.roomId;
  return `<span class="plantpick">${p.name} · ${vnd(p.price)}
    <button type="button" onclick="addItemToRoom('${p.id}','${rid}')">+ Thêm</button>
    <a href="${buyUrl(p)}" target="_blank" rel="noopener noreferrer">Mua ↗</a></span>`;
}
...
function runChecks(rid){
  ...
  // 4. chắn cửa — kiểm tra cả đồ nội thất DI CHUYỂN ĐƯỢC (items) lẫn đồ CỐ
  // ĐỊNH gắn liền phòng (r.fixtures, vd tủ bếp/tủ âm tường khai báo trong mặt
  // bằng) — thiếu vế sau thì mặt bằng tự thiết kế lỡ đặt bếp đè lên cửa vẫn
  // báo "OK" vì thuật toán xếp đồ không đụng tới fixtures, chỉ người tự kiểm
  // bằng mắt mới thấy.
  const doors = (r.openings||[]).filter(o => o.type === 'door' || o.type === 'open' || o.type === 'glass');
  const blocked = [];
  doors.forEach(o => {
    // cửa kính lô gia trượt (không có cánh xoay) nên dùng vùng lối đi thẳng
    ...
    (r.fixtures||[]).forEach(f => {
      if (overlap({x:f.x, y:f.y, w:f.w, d:f.d}, sw)) blocked.push(f.name + ' (cố định) chắn ' + o.label.toLowerCase());
    });
  });
  ...
  /* ---- phong thuỷ ---- */
  // ưu tiên năm sinh NGƯỜI Ở ĐÚNG PHÒNG NÀY (khai báo qua ghi chú), chưa
  // khai báo thì dùng năm sinh chung ở mục 1 — xem yearForRoom().
  const occupant = S.need.occupants[rid];
  const roomYear = yearForRoom(rid);
  const cm = cungMenh(roomYear);
  const na = napAmOf(roomYear);
  const plantPicks = na.cay.map(name => CATALOG.find(x => x.cat==='cay' && x.name===name)).filter(Boolean);
  const plantHtml = plantPicks.length
    ? '<span class="plantrow">' + plantPicks.map(pl => suggestPickHtml(pl, rid)).join('') + '</span>'
    : na.cay.join(', ');
  // tượng hợp mệnh — cùng cơ chế với cây hợp mệnh ở trên (na.tuong khai báo
  // trong MENH_INFO, data.js), thêm đúng vào PHÒNG đang xét (rid), không
  // phải phòng đang mở trên màn hình, để đúng người/phòng dù xem chỗ khác.
  const tuongPicks = (na.tuong||[]).map(name => CATALOG.find(x => x.cat==='tuong' && x.name===name)).filter(Boolean);
  const tuongHtml = tuongPicks.length
    ? '<span class="plantrow">' + tuongPicks.map(tg => suggestPickHtml(tg, rid)).join('') + '</span>'
    : '';
  out.push({lv:'ok',
    t:'Mệnh ' + na.ten + ' (' + na.label + ') — năm ' + na.canChi
      + (occupant ? ' — phòng của ' + occupant.label.toLowerCase() : ''),
    s:(occupant && !occupant.year ? '(Chưa rõ năm sinh riêng của ' + occupant.label.toLowerCase() + ', đang tạm tính theo năm sinh chung ở mục 1.) ' : '')
      + 'Màu hợp: ' + na.mauHop.map(m=>m.name).join(', ') + '. Nên tránh: ' + na.mauKy.map(m=>m.name).join(', ') + '. '
      + na.ly + ' Cây hợp mệnh: ' + plantHtml
      + (tuongHtml ? ' Tượng hợp mệnh: ' + tuongHtml : '')});

  /* phong thuỷ CẤU TRÚC quanh cửa chính — áp dụng cho cả căn hộ (không riêng
     phòng đang xem), suy ra thuần hình học từ plan.entry/openings/fixtures
     nên tự chạy đúng với MỌI mặt bằng, kể cả mặt bằng thêm sau này. */
  const entryInfo = findEntryRoom();
  if (entryInfo){
    const eR = entryInfo.room, eO = entryInfo.opening;
    const corridor = sightlineCorridor(eR, eO);
    const oppWall = OPP_WALL[eO.wall];
    /* cây cao nhất còn vừa phòng — vật cản tầm nhìn tự nhiên, đúng luôn vật
       phẩm phong thuỷ, dùng chung cho các gợi ý "che chắn" quanh cửa chính. */
    const tallPlant = CATALOG.filter(p => p.cat==='cay' && fitsRoom(p, eR)).sort((a,b) => b.h-a.h)[0];

    const oppLogia = (eR.openings||[]).find(o =>
      o.wall === oppWall && (o.type==='glass' || o.type==='railing') && overlap(openingRect(o, eR), corridor));
    out.push(oppLogia
      ? {lv:'warn', t:'Cửa chính thẳng trục ra ' + oppLogia.label.toLowerCase(),
         s:'Khí vào thẳng ra luôn hết một mạch ở phòng "' + eR.name + '", dân gian gọi "xuyên đường sát" — khó tụ tài. '
           + 'Đặt 1 cây cao hoặc kệ tủ thấp chắn giữa lối vào và lô gia là hoá giải được, không cần đổi kết cấu.'
           + (tallPlant ? ' Gợi ý: ' + suggestPickHtml(tallPlant, eR.id) : '')}
      : {lv:'ok',  t:'Cửa chính không xuyên thẳng ra lô gia / ban công', s:''});

    const oppDoor = (eR.openings||[]).find(o =>
      o.wall === oppWall && o.type === 'door' && overlap(openingRect(o, eR), corridor));
    const oppRoom = oppDoor ? neighborRoomThrough(eR, oppDoor) : null;
    out.push(oppRoom && oppRoom.type === 'wc'
      ? {lv:'err', t:'Cửa chính đối diện thẳng cửa nhà vệ sinh',
         s:'Vừa mở cửa vào phòng "' + eR.name + '" đã thấy cửa WC là điều tối kỵ trong phong thuỷ. '
           + 'Cách sửa đúng gốc là đổi hướng bản lề cửa WC; nếu chưa sửa được kết cấu, che tạm bằng 1 cây cao ngay lối vào.'
           + (tallPlant ? ' Gợi ý: ' + suggestPickHtml(tallPlant, eR.id) : '')}
      : {lv:'ok',  t:'Cửa chính không đối diện cửa nhà vệ sinh', s:''});

    const seesBep = (eR.fixtures||[]).some(f => f.name === 'Bếp' && overlap({x:f.x, y:f.y, w:f.w, d:f.d}, corridor));
    out.push(seesBep
      ? {lv:'warn', t:'Cửa chính nhìn thẳng vào bếp',
         s:'Bếp ở phòng "' + eR.name + '" lộ ngay trước cửa ra vào, dân gian cho là dễ hao tài. Che một phần bếp khỏi tầm nhìn từ cửa chính bằng vách/tủ/rèm.'
           + (tallPlant ? ' Gợi ý: ' + suggestPickHtml(tallPlant, eR.id) : '')}
      : {lv:'ok',  t:'Cửa chính không nhìn thẳng vào bếp', s:''});
  }
  ...
  if (bed){
    ...
    const goodWalls = good ? [] : ['N','E','S','W'].filter(w => w !== e && cm.tot.includes(huongCua(w)));
    out.push({lv: good ? 'ok' : 'warn',
      t:'Đầu giường quay hướng ' + h + (good ? ' — hợp mệnh' : ' — chưa hợp mệnh'),
      s:(occupant ? occupant.label : 'Bạn') + ' thuộc cung ' + cm.ten + ' (' + cm.nhom + '). Hướng tốt: ' + cm.tot.join(', ') + '.'
        + (good ? '' : goodWalls.length
            ? ' Nên kê đầu giường sát tường hướng ' + goodWalls.map(huongCua).join(' hoặc ') + '. '
              + '<button type="button" onclick="fixBedDirection()">Xoay đầu giường vào hướng hợp mệnh ↻</button>'
            : ' Phòng này không có tường nào hướng hợp mệnh — chỉ có thể hoá giải bằng vật phẩm phong thuỷ, khó đổi bằng cách kê lại giường.')});
    ...
    if (mirrors.length){
      const facing = mirrorsFacingBedFoot(bed, r, footWall);
      out.push(facing.length
        ? {lv:'warn', t:'Gương đối diện chân giường',
           s:'Dân gian cho rằng gương soi thẳng vào người đang ngủ dễ giật mình, hao khí. '
             + '<button type="button" onclick="fixMirrorFacingBed()">Chuyển gương sang tường bên ↻</button>'}
  ...
  if (altar){
    ...
    const suggestion = CATALOG.filter(p => p.cat==='bantho' && fitsRoom(p, r)).sort((a,b) => a.price-b.price)[0];
    out.push({lv:'warn', t:'Chưa bố trí bàn thờ',
      s:'Phần lớn hộ gia đình Việt cần chỗ đặt bàn thờ trong phòng khách.'
        + (suggestion ? ' Gợi ý rẻ nhất còn vừa phòng: ' + suggestPickHtml(suggestion) : ' Có sẵn trong danh mục.')});
  }
...
function blockedZones(avoidWindow){
  ...
  (r.openings||[]).forEach(o => {
    ...
    /* cửa lô gia (glass) LÀ LỐI ĐI — phải luôn chặn như door/open, không được
       gộp chung với "window" (chỉ là nguồn sáng, không phải lối đi) rồi tuỳ
       vào avoidWindow. Trước đây gộp chung nên khi 1 lệnh đặt đồ tắt
       avoidWindow (vd kệ tivi: avoidWindow:false) thì cửa lô gia bị bỏ ngỏ,
       tivi/sofa/bàn trà/đèn có thể đè thẳng lên lối ra ban công. */
    else if (o.type === 'glass') z.push(passRect(o, r, 60));
...
/* rot lưu trong item là góc xoay THEO CHIỀU quy ước "mặt trước xa tường"
   dùng chung cho cả vạch chỉ hướng 2D (drawItems) lẫn xoay khối 3D
   (td_furniture, g.rotation.y = -it.rot). Test tay bằng lượng giác: với
   công thức xoay đang dùng, tường Đông/Tây phải nhận giá trị NGƯỢC với
   tường Bắc/Nam so với trực giác (N:0,S:180 đối xứng thẳng, nhưng E/W
   phải là 90/270 đảo ngược nhau) — nếu gán như "hiển nhiên" (E:270,W:90)
   thì mặt trước đồ vật quay ngược vào tường thay vì ra giữa phòng. */
...
const deskRoomId = () => (S.need.oneBedOnly && plan().rooms.some(r => r.id === 'plus1')) ? 'plus1' : 'master';
...
/* năm sinh dùng để tính phong thuỷ cho 1 phòng cụ thể — ưu tiên người đã
   khai báo riêng cho phòng đó qua ghi chú (S.need.occupants), nếu chưa khai
   báo thì dùng năm sinh chung ở mục 1 (S.year) như trước giờ, để mặt bằng
   không có khai báo riêng (vd căn 1PN+1) không đổi hành vi. */
function yearForRoom(rid){
...
function parseNeedNotes(text){
  ...
  /* giữ nguyên chữ hoa/thường của câu gốc (clausesRaw) để hiện lại đúng như
     ...
     Chỉ còn các câu KHÔNG PHẢI khai báo người (leftoverSentences) mới tách
     tiếp theo dấu phẩy như trước giờ. */
  const clausesRaw = leftoverSentences.join('. ').split(/[,.;\n]+/).map(s => s.trim()).filter(Boolean);
  ...
  const parsed = [];
  Object.keys(n.occupants).forEach(rid => {
    const occ = n.occupants[rid], rr = roomById(rid);
    if (!rr) return;
...
function buildCatalog(){
  ...
  $('#catList').innerHTML = list.map(p => `
    <div class="item">
      ...
      <a class="buy" href="${buyUrl(p)}" target="_blank" rel="noopener noreferrer">Tìm mua ↗</a>
    </div>`).join('');
  $('#catList').querySelectorAll('.itemAdd').forEach(b => b.onclick = () => addItem(b.dataset.id));
}

function addItem(id){ addItemToRoom(id, S.roomId); }
/* thêm đồ vào 1 phòng CHỈ ĐỊNH (không nhất thiết là phòng đang xem) — dùng
   cho các gợi ý "phong thuỷ cấu trúc" quanh cửa chính, vốn hiện trên panel
   của MỌI phòng chứ không riêng phòng có cửa chính, nên nút "+Thêm" phải
   luôn thêm đúng vào phòng cần sửa, không phải phòng đang mở trên màn hình. */
function addItemToRoom(id, roomId){
  const p = byId(id), r = roomById(roomId), list = itemsIn(roomId);
  ...
  list.push(it);
  if (roomId === S.roomId) S.sel = it.uid;
  renderAll();
}

function renderChecks(){
  /* đẩy lỗi/cảnh báo lên đầu danh sách thay vì để lẫn giữa các dòng "OK" —
     vấn đề cần chú ý phải là thứ nhìn thấy ngay, không phải dò cả danh sách
     mới thấy. Thứ tự runChecks() trả về không đổi (chỗ khác vẫn dùng được),
     chỉ sắp lại khi HIỂN THỊ. */
  const rank = {err:0, warn:1, ok:2};
  const sorted = runChecks().slice().sort((a,b) => rank[a.lv]-rank[b.lv]);
  $('#checks').innerHTML = sorted.map(c => `
    <div class="check ${c.lv==='ok'?'ok':c.lv==='warn'?'warn':'err'}">
...
function td_furniture(it, ox, oy){
  ...
  switch(p.cat){
    case 'sofa': {
      ...
      if (p.shape === 'corner'){
        /* sofa góc chữ L: 1 khối ngồi dài dựa tường sau + 1 khối chaise vuông
           góc nhô ra ở đầu bên phải — 2 khối chồng mí tạo hình L thay vì chữ
           nhật đơn như sofa thường. */
        const segD = Math.min(D*.58, D-.4), chW = Math.min(W*.42, W-.4);
        const baseZ = -D/2 + segD/2;
        add(rb(W, seat*.55, segD, .05, fab, 0, seat*.28, baseZ));
        add(rb(chW, seat*.55, D, .05, fab, W/2-chW/2, seat*.28, 0));
        add(rb(W-chW, H*.9, .12, .06, fab, -chW/2, H*.45, -D/2+.06));
        add(rb(armW, H*.62, segD, .06, fab, -W/2+armW/2, H*.31, baseZ));
        const nH = (W-chW)>1.2 ? 3 : 2, runW = W-chW-armW*.6;
        for(let i=0;i<nH;i++){
          const cush=rb(runW/nH*.92, H*.20, segD*.6, .07, fab, -W/2+armW*.6+runW/nH*(i+.5), seat*.55+H*.10, baseZ+segD*.08);
          cush.rotation.x=-.03; add(cush);
        }
        const nV = D>1.4 ? 2 : 1;
        for(let j=0;j<nV;j++){
          const cush=rb(chW*.88, H*.20, D/nV*.9, .07, fab, W/2-chW/2, seat*.55+H*.10, -D/2+D/nV*(j+.5));
          cush.rotation.x=-.03; add(cush);
        }
      } else if (p.shape === 'armchair'){
        /* ghế bành 1 chỗ: tay vịn bọc hết 2 bên, chỉ 1 đệm ngồi duy nhất —
           không chia nhiều đệm như sofa văng nhiều chỗ. */
        const armW2=W*.22;
        add(rb(W, seat*.55, D, .08, fab, 0, seat*.28, 0));
        add(rb(W, H*.95, D*.24, .08, fab, 0, H*.475, -D/2+D*.12));
        add(rb(armW2, H*.62, D*.85, .07, fab, -W/2+armW2/2, H*.31, D*.02));
        add(rb(armW2, H*.62, D*.85, .07, fab,  W/2-armW2/2, H*.31, D*.02));
        const cush=rb(W-armW2*2-.04, H*.22, D*.60, .08, fab, 0, seat*.55+H*.10, D*.05);
        cush.rotation.x=-.03; add(cush);
      } else if (p.shape === 'bench'){
        /* sofa băng không tay vịn: chỉ có lưng tựa thấp, 2 bên hở hoàn toàn. */
        add(rb(W, seat*.55, D, .05, fab, 0, seat*.28, 0));
        add(rb(W*.98, H*.85, D*.16, .06, fab, 0, H*.42, -D/2+D*.08));
        const n=W>1.6?3:2, cw=W/n;
        for(let i=0;i<n;i++){
          const cush=rb(cw*.94, H*.20, D*.64, .07, fab, -W/2+cw*(i+.5), seat*.55+H*.10, D*.06);
          cush.rotation.x=-.03; add(cush);
        }
      } else {
        add(rb(W, seat*.55, D, .05, fab, 0, seat*.28, 0));
    ...
    case 'giuong': {
      if (p.shape === 'upholstered'){
        /* giường bọc da/vải: đế + đầu giường đều bọc fab thay vì gỗ, đầu
           giường cao bản có sọc chần bông (tufting) thay vì tấm gỗ phẳng. */
        add(rb(W, H*.5, D, .05, fab, 0, H*.25, 0));
        add(rb(W*.97, H*.42, D*.95, .05, M({color:0xF3EFE6, roughness:.95}), 0, H*.5+H*.21, 0));
        add(rb(W, .74, .10, .05, fab, 0, H*.5+.27, -D/2+.05));
        const nRib=Math.max(4, Math.round(W/.22));
        for(let i=0;i<nRib;i++) add(bx(.012, .70, .012, M({color:dark, roughness:.9}), -W/2+W/nRib*(i+.5), H*.5+.27, -D/2+.095));
        const duvet=rb(W*.99, .07, D*.62, .04, M({color:c, map:fabricTex('#'+c.getHexString()), roughness:.97}), 0, H*.5+H*.42+.02, D*.16); add(duvet);
      ...
      } else if (p.shape === 'storage'){
        /* giường ngăn kéo: đế cao hơn, có mặt ngăn kéo + tay nắm lộ ra ở cạnh
           chân giường, đầu giường thấp gọn hơn kiểu mặc định. */
        add(rb(W, H*.6, D, .02, woodD, 0, H*.3, 0));
        const nDr=W>1.4?3:2;
        for(let i=0;i<nDr;i++){
          const x=-W/2+W/nDr*(i+.5);
          add(bx(W/nDr*.90, H*.42, .012, wood, x, H*.36, D/2+.007));
          add(bx(W/nDr*.5, .02, .02, metal, x, H*.36, D/2+.02));
        }
        add(rb(W*.97, H*.30, D*.95, .05, M({color:0xF3EFE6, roughness:.95}), 0, H*.6+H*.15, 0));
        add(rb(W, .42, .07, .04, fab, 0, H*.6+.13, -D/2+.035));
        const duvet=rb(W*.99, .07, D*.62, .04, M({color:c, map:fabricTex('#'+c.getHexString()), roughness:.97}), 0, H*.6+H*.30+.02, D*.16); add(duvet);
        for(const sx of (W>1.4?[-.26,.26]:[0])){
          const pil=rb(W*.36, .12, .3, .06, M({color:0xFAF7F1, roughness:.96}), sx*W, H*.6+H*.30+.05, -D/2+.28);
          pil.rotation.x=-.16; add(pil);
        }
      } else {
    ...
    case 'tuquanao': {
      if (p.shape === 'sliding'){
        /* tủ cửa lùa: 2 cánh chồng mí theo chiều sâu (1 cánh nhô ra trước 1
           cánh) thay vì các cánh cửa bản lề tách rời cạnh nhau, tay nắm là
           thanh dọc mảnh thay vì núm tròn. */
        add(rb(W,H,D,.015, wood, 0, H/2, 0));
        const half=W/2;
        add(bx(half*1.02, H*.97, .014, woodD, -W/4, H/2, D/2+.010));
        add(bx(half*1.02, H*.97, .014, woodD,  W/4, H/2, D/2+.020));
        add(bx(.016, H*.5, .02, metal, -.02, H*.52, D/2+.030));
        add(bx(.016, H*.5, .02, metal,  half-.06, H*.52, D/2+.010));
      } else {
        add(rb(W,H,D,.015, wood, 0, H/2, 0));
        const n=W>1.4?3:2;
        for(let i=0;i<n;i++){
    ...
    case 'ketivi': {
      if (p.shape === 'cabinet'){
        /* tủ tivi kết hợp tủ rượu: cánh tủ kín thay vì mặt hở lộ khoang chứa. */
        add(rb(W,H,D,.02, wood, 0, H/2, 0));
        const n=2;
        for(let i=0;i<n;i++){
          const x=-W/2+W/n*(i+.5);
          add(bx(W/n*.94, H*.9, .012, woodD, x, H/2, D/2+.008));
          add(cyl(.008,.008,.14, metal, x + (i===0? W/n*.36 : -W/n*.36), H*.55, D/2+.03));
        }
      } else {
        add(rb(W,H,D,.02, wood, 0, H/2, 0));
    ...
    case 'bantra': case 'banan': case 'banlam': {
      ...
      if (p.shape === 'round'){
        /* mặt bàn tròn/oval: trụ tròn kéo giãn theo tỉ lệ W:D thay vì mặt
           chữ nhật, đứng trên 1 chân trụ giữa thay vì 4 chân góc. */
        const rIn = Math.min(W,D)/2;
        const top = cyl(rIn, rIn, .045, topM, 0, H-.0225, 0);
        top.scale.set(W/(2*rIn), 1, D/(2*rIn)); add(top);
        add(cyl(.05, .09, H-.045, metal, 0, (H-.045)/2, 0));
        add(cyl(rIn*.5, rIn*.5, .02, metal, 0, .015, 0));
      } else if (p.shape === 'nested'){
        /* bàn xếp lồng: 2 mặt bàn kích thước khác nhau lệch chéo nhau, mỗi
           mặt 4 chân riêng — không phải 1 mặt bàn duy nhất. */
        const w2=W*.62, d2=D*.62, h2=H*.72;
        add(rb(W,.04,D,.012, topM, -W*.08, H-.02, -D*.08));
        const lx=W/2-.06, lz=D/2-.06;
        [[-lx,-lz],[lx,-lz],[-lx,lz],[lx,lz]].forEach(([x,z])=>add(cyl(.018,.022,H-.04, metal, x-W*.08,(H-.04)/2,z-D*.08)));
        add(rb(w2,.035,d2,.012, topM, W*.14, h2-.0175, D*.14));
        const lx2=w2/2-.05, lz2=d2/2-.05;
        [[-lx2,-lz2],[lx2,-lz2],[-lx2,lz2],[lx2,lz2]].forEach(([x,z])=>add(cyl(.015,.018,h2-.035, metal, x+W*.14,(h2-.035)/2,z+D*.14)));
      } else {
    ...
    case 'trangdiem': {
      /* bàn trang điểm: luôn có gương gắn liền phía sau mặt bàn — khác hẳn
         bàn làm việc thường (không có mặt phản chiếu). Sản phẩm cao (v3, có
         đèn LED viền gương) thì mặt bàn vẫn giữ đúng độ cao bàn chuẩn
         (~0,75 m), phần chênh lệch còn lại (H tổng - độ cao bàn) là khung
         gương cao hơn, không phải bàn cao hơn. */
      const deskH = Math.min(H, .76);
      add(rb(W, .04, D, .012, wood, 0, deskH-.02, 0));
      const lx=W/2-.06, lz=D/2-.10;
      [[-lx,-lz],[lx,-lz],[-lx,lz],[lx,lz]].forEach(([x,z])=>add(cyl(.018,.022,deskH-.04, metal, x,(deskH-.04)/2,z)));
      const mirrorH = Math.max(H-deskH, .28);
      const frameM = M({color:dark, roughness:.5, metalness:.25});
      const mirrorM = M({color:0xC9D3D8, roughness:.03, metalness:1});
      const mw = W*.55;
      add(rb(mw, mirrorH, .03, frameM, 0, deskH+mirrorH/2, -D/2+.03));
      add(bx(mw*.86, mirrorH*.88, .01, mirrorM, 0, deskH+mirrorH/2, -D/2+.03+.012));
      if (H > .9){
        const ledM = M({color:0xFFF6E2, emissive:0xFFDCA0, emissiveIntensity:.8, roughness:.4});
        [-1,1].forEach(s => add(bx(.012, mirrorH*.85, .012, ledM, s*mw*.46, deskH+mirrorH/2, -D/2+.045)));
      }
      break;
    }
    case 'ghe': {
    ...
    case 'bantho': {
      /* chiều cao treo (p.mount) áp dụng chung ở cuối hàm (g.position.y) cho
         MỌI hạng mục — khối này chỉ dựng hình tương đối so với đế của chính nó. */
      const hh=Math.max(H,.12);
      add(rb(W,hh,D,.02, woodD, 0, hh/2, 0));
      const top=hh;
    ...
    case 'tuong': {
      /* tượng phong thuỷ: 2 dáng riêng theo chiều cao — linh vật để bàn
         (tỳ hưu/thiềm thừ/rùa, thấp+dài, không bệ) và tượng đứng/ngồi có bệ
         (Phật/Quan Công/cá chép hoá rồng) — không dùng chung 1 khối hộp. */
      const l = c.getHSL({h:0,s:0,l:0}).l;
      const bodyM = M({color:c, roughness:.4, metalness: l < .35 ? .55 : .1});
      if (H < .16){
        add(cyl(W*.28, W*.34, H*.55, bodyM, 0, H*.27, 0));
        const head = new THREE.Mesh(new THREE.SphereGeometry(Math.min(W,D)*.32, 16, 12), bodyM);
        head.position.set(0, H*.6, -D*.28); head.scale.set(1,.8,1.15); head.castShadow=true; add(head);
      } else {
        add(cyl(Math.max(W,D)*.42, Math.max(W,D)*.48, H*.10, M({color:0x8A7048, roughness:.5}), 0, H*.05, 0));
        const body = new THREE.Mesh(new THREE.CylinderGeometry(W*.30, W*.42, H*.62, 14), bodyM);
        body.position.set(0, H*.10+H*.31, 0); body.castShadow=true; add(body);
        const head = new THREE.Mesh(new THREE.SphereGeometry(W*.30, 16, 12), bodyM);
        head.position.set(0, H*.10+H*.62+W*.24, 0); head.castShadow=true; add(head);
      }
      break;
    }
    case 'tranh': {
      /* tranh treo tường: khung + mặt tranh nhám (không phản chiếu như
         gương) + 1 mảng màu nhấn gợi có bố cục, để phân biệt rõ với gương. */
      const frameM2 = M({color:0x2E251C, roughness:.75});
      const canvasM = M({color:c, roughness:.95});
      const accentM = M({color:light, roughness:.9});
      add(rb(W, H, Math.max(D,.025), .01, frameM2, 0, H/2, 0));
      add(bx(W*.9, H*.86, .01, canvasM, 0, H/2, D/2-.006));
      add(bx(W*.55, H*.30, .011, accentM, -W*.08, H*.36, D/2-.004));
      break;
    }
...
function td_fixtures(){
  ...
  // thiết bị vệ sinh trong WC — áp dụng cho MỌI phòng type 'wc' (không chỉ
  // phòng có id đúng bằng 'wc'), để căn hộ nhiều WC (vd GS1-12) không bị bỏ sót
  P.rooms.filter(r=>r.type==='wc').forEach(wc=>{
    const X=x=>(wc.x+x)/100, Z=z=>(wc.y+z)/100;
    const cx=Math.min(75, wc.w-20), cy=Math.min(150, wc.d-40);
    g.add(bx(.36,.42,.62, white, X(cx), .21, Z(cy)));
    g.add(rb(.37,.16,.42,.07, white, X(cx), .46, Z(cy+13)));
    g.add(bx(.16,.55,.22, white, X(cx), .48, Z(cy-30)));
    const sx=Math.min(45, wc.w-20), sy=Math.min(228, wc.d-20);
    g.add(rb(.52,.13,.38,.05, white, X(sx), .80, Z(sy)));
    g.add(cyl(.016,.016,.16, metal, X(sx), .92, Z(sy-13)));
    g.add(bx(.5,.7,.02, new THREE.MeshPhysicalMaterial({color:0xE6F0F2,transparent:true,opacity:.2,roughness:.05,metalness:0}), X(sx), 1.5, Z(sy+2)));
    const glass=new THREE.MeshPhysicalMaterial({color:0xDCEAF0,transparent:true,opacity:.14,roughness:.04});
    g.add(bx(.02,2.0,Math.min(1.0, wc.d/100), glass, X(Math.min(142, wc.w-20)), 1.0, Z(Math.min(60, wc.d/2))));
  });
  return g;
}
...
function td_decor(){
  ...
  // cây trang trí — áp dụng cho MỌI lô gia (type 'logia'), không chỉ phòng
  // có id đúng bằng 'logia', để căn hộ nhiều lô gia (vd GS1-12) không bị bỏ sót
  P.rooms.filter(r=>r.type==='logia').forEach(lg=>{
    [[.35,.5],[.6,1.6],[.4,2.4]].forEach(([px,pz],i)=>{
      if (pz*100 > lg.d-20 || px*100 > lg.w-20) return;
